...
class Cart extends MX_Controller {
    ...
    public function _attempt_draw_checkout_btn($boat_rental_query, $lesson_query) {
    ...
    public function _draw_checkout_btn_real($boat_rental_query, $lesson_query) {
        $this->load->module('paypal');
        $this->paypal->_draw_checkout_btn($boat_rental_query, $lesson_query);
    }

    public function _draw_checkout_btn_fake($boat_rental_query, $lesson_query) {
    ...
    public function _draw_cart_content($boat_rental_query, $lesson_query, $user_type) {
    ...
    public function _fetch_boat_rental_cart_content($session_id, $shopper_id, $table) {
        $this->load->module('timedate');
        // get contents for which the boat has been removed
        $mysql_query = "SELECT * FROM $table";
        $boat_expired_query = "";
        if ($shopper_id > 0) {
            $mysql_query .= " WHERE (session_id = ? OR shopper_id = ?)";
            $boat_expired_query = $this->db->query($mysql_query, array($session_id, $shopper_id));
        } else {
            $mysql_query .= " WHERE session_id = ?";
            $boat_expired_query = $this->db->query($mysql_query, array($session_id));
        }

        if ($boat_expired_query->num_rows() > 0) {
            if ($this->session->has_userdata('overdue_items')) {
                $overdue_items = $this->session->userdata('overdue_items');
            } else {
                $overdue_items = array();
            }
            $mysql_query = "SELECT * FROM boat_rental WHERE id = ?";
            foreach ($boat_expired_query->result() as $row) {
                ...
                $boat_id = $row->boat_id;
                $boat_query = $this->db->query($mysql_query, array($row->boat_id));
                if ($boat_query->num_rows() == 0) {
                    array_push($overdue_items, $row->boat_name.' for '.$date.': '.$start_time.' - '.$end_time.' because the boat has been removed.');
        ...
        if ($overdue_query->num_rows() > 0) {
            ...
            foreach ($overdue_query->result() as $row) {
                ...
                $boat_id = $row->boat_id;
                $mysql_query = "SELECT * FROM boat_rental WHERE id = ?";
                $boat_query = $this->db->query($mysql_query, array($boat_id));
                if ($boat_query->num_rows() == 0) {
                    array_push($overdue_items, $row->boat_name.' for '.$date.': '.$start_time.' - '.$end_time.' because the boat has been removed.');
                } else {
    ...
    public function _fetch_lesson_cart_content($session_id, $shopper_id, $table) {
        $this->load->module('timedate');
        // get contents for which the lesson schedule has been removed
        $mysql_query = "SELECT * FROM $table";
        $removed_lesson_schedule_query = "";
        if ($shopper_id > 0) {
            $mysql_query .= " WHERE (session_id = ? OR shopper_id = ?)";
            $removed_lesson_schedule_query = $this->db->query($mysql_query, array($session_id, $shopper_id));
        } else {
            $mysql_query .= " WHERE session_id = ?";
            $removed_lesson_schedule_query = $this->db->query($mysql_query, array($session_id));
        }

        if ($removed_lesson_schedule_query->num_rows() > 0) {
            if ($this->session->has_userdata('overdue_items')) {
                $overdue_items = $this->session->userdata('overdue_items');
            } else {
                $overdue_items = array();
            }
            $mysql_query = "SELECT * FROM lesson_schedules WHERE id = ?";
            foreach ($removed_lesson_schedule_query->result() as $row) {
                $lesson_schedule_query = $this->db->query($mysql_query, array($row->schedule_id));
                if ($lesson_schedule_query->num_rows() == 0) {
                    ...
                    // push a string into the array
                    array_push($overdue_items, $row->lesson_name.' for '.$date.': '.$start_time.' - '.$end_time.' because the schedule has been removed.');
                    $delete_statement = "DELETE FROM $table WHERE id = ?";
                    $this->db->query($delete_statement, array($row->id));
                    // put the array into the session.
                    $this->session->set_userdata('overdue_items', $overdue_items);
