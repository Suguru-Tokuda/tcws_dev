      ...
      <table class="table" style="margin-top: 36px;">
        ...
        if($boat_rental_query->num_rows() > 0){
          foreach ($boat_rental_query->result() as $row) {
            ...
            <tr style="margin-top: 12px;">
              <td>
                <?php if ($picture_name != '') { ?>
                  <img style="width: 300px;" src="<?= base_url() ?>media/boat_rental_small_pics/<?= $picture_name ?>" title="<?= $picture_name ?>">
                <?php } else {
        ...
        if($lesson_query->num_rows() > 0){
          ...
          foreach ($lesson_query->result() as $row) {
            ...
            $picture_name = $this->lesson_pics->get_first_picture_name($lesson_id);
            ...
            <tr style="margin-top: 12px;">
              <td>
                ...
                if ($picture_name != '') {
                  ?>
                  <img style="width: 300px;" src="<?= base_url() ?>media/lesson_small_pics/<?= $picture_name ?>" title="<?= base_url() ?>media/lesson_small_pics/<?= $picture_name ?>">
                  <?php
                } else {
                  echo "No image preview available";
                }
