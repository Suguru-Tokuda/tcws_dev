...
$lessons_url = base_url().'lessons/view_your_lessons/'.$pagination_limit_for_lessons;
$boats_url = base_url().'boat_rental/view_your_rental_boats/'.$pagination_limit_for_lessons;
...
$logout_url = base_url().'youraccount/logout';
 ...
 <nav class="list-group">
   <a class="list-group-item <?php if ($url_segment === "listed_items") { echo "active"; } ?> <?php if ($num_of_items > 0) {?>with-badge<?php }?>" href="<?= $items_url ?>">
     <i class="icon-bag"></i>Your Items<?php if ($num_of_items > 0) { ?><span class="badge badge-primary badge-pill"><?= $num_of_items ?></span><?php }?>
   </a>
   <a class="list-group-item <?php if ($url_segment === "lessons") { echo "active"; } ?> <?php if ($num_of_lessons > 0) {?>with-badge<?php }?>" href="<?= $lessons_url ?>">
     <i class="icon-tag"></i>Your Booked Lessons<?php if ($num_of_lessons > 0) { ?><span class="badge badge-primary badge-pill"><?= $num_of_lessons ?></span><?php }?>
   </a>
   <a class="list-group-item <?php if ($url_segment === "boat_rental") { echo "active"; } ?> <?php if ($num_of_rental_boats > 0) {?>with-badge<?php }?>" href="<?= $boats_url ?>">
     <i class="icon-anchor"></i>Your Rental Boats<?php if ($num_of_rental_boats > 0) { ?><span class="badge badge-primary badge-pill"><?= $num_of_rental_boats ?></span><?php }?>
   </a>
